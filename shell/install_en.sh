...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            if [[ "${release}" == "alpine" ]]; then
                wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
            else
                wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
            fi
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        if [[ "${release}" == "alpine" ]]; then
            wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        else
            wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
        fi
...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -O /etc/v2ray-agent/install.sh --no-check-certificate "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/shell/install_en.sh"
    else
        wget -c -q "${wgetShowProgressStatus}" -O /etc/v2ray-agent/install.sh --no-check-certificate "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/shell/install_en.sh"
    fi
    ...
    echoContent skyBlue "wget -O /root/install.sh --no-check-certificate https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/shell/install_en.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
            ...
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
                ...
                #clashMeta
                    ...
                    if [[ -f "/etc/v2ray-agent/subscribe_local/sing-box/${email}" ]]; then
                        ...
                        if [[ "${release}" == "alpine" ]]; then
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/documents/sing-box.json"
                        else
                            wget -O "/etc/v2ray-agent/subscribe/sing-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/documents/sing-box.json"
                        fi

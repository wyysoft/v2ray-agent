...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/install.sh"
    else
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/install.sh"
    fi
    ...
    echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/wyysoft/v2ray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
...
manageAccount() {
    ...
    echoContent yellow "6.生成小火箭规则地址与二维码"
    echoContent red "=============================================================="
